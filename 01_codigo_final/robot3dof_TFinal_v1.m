%% ROBOT ANTROPOMORFICO 3GDL - TRABAJO FINAL v1
% -------------------------------------------------------------------------
% Curso      : Robotica y Sistemas Autonomos
% Entregable : Version preliminar para presentacion del Trabajo Final
% Archivo    : robot3dof_TFinal_v1.m
%
% Punto de partida:
% Este archivo se construye a partir del trabajo parcial:
%   robot3dof_paper_TParcial_g2.m
% donde ya se implemento:
%   1) Cinematica directa mediante DH
%   2) Cinematica inversa geometrica
%   3) Jacobiano y singularidades
%   4) Control cinematico por pseudoinversa
%   5) Simulacion 3D de trayectoria del efector final
%
% Ampliacion para Trabajo Final:
% El enunciado del Trabajo Final solicita:
%   - Control de Posicion: PID no lineal.
%   - Control de Movimiento: PD con precompensacion.
%   - Control de Movimiento: Control por Par Calculado.
%   - Desarrollo de trayectoria autonoma con obstaculos.
%   - Simulacion en Matlab/Simulink.
%
% Objetivo de esta version v1:
%   A) Mantener la base cinematica del parcial.
%   B) Agregar un modelo dinamico M(q), C(q,dq), G(q).
%   C) Simular y comparar tres controladores dinamicos.
%   D) Incorporar planeacion autonoma con obstaculos mediante A*.
%   E) Generar graficas y metricas para una presentacion preliminar.
%
...
% 1) El paper base usado en el parcial no proporciona una tabla completa de
%    masas, inercias y centros de masa. Por ello, esta version usa parametros
%    fisicos asumidos y claramente declarados para fines de simulacion.
% 2) Esta decision debe consultarse con el docente. Si el docente exige
%    derivacion dinamica completa desde Lagrange con parametros medidos, se
%    debera reemplazar esta dinamica simplificada por una derivacion formal.
% 3) Este archivo es una version preliminar funcional para explicar el flujo
%    completo del Trabajo Final. No reemplaza aun el informe ni el modelo
%    Simulink, pero deja el nucleo computacional listo.
%
% Preguntas sugeridas al docente:
...
%   Q2. ¿La simulacion puede presentarse en script MATLAB con graficas y
%       comparacion numerica, o es obligatorio adjuntar tambien un modelo
%       Simulink editable?
%   Q3. ¿La planeacion autonoma con obstaculos puede realizarse en el espacio
%       cartesiano del efector usando A* y luego convertir la ruta mediante
%       cinematica inversa?
%   Q4. ¿La comparacion de controladores debe hacerse sobre la misma
%       trayectoria planeada o basta con casos separados de regulacion y
%       seguimiento?
%
% -------------------------------------------------------------------------
...
% ================================================================
% Geometria tomada del paper base usado en el parcial.
...
% Estos valores deben declararse en el informe como supuestos de simulacion.
...
% Limites de torque para evitar resultados numericamente absurdos.
% No representan necesariamente un actuador real; son una proteccion numerica.
robot.tau_max = [80; 80; 60]; % [N*m]

fprintf('============================================================\n');
fprintf(' ROBOT ANTROPOMORFICO 3GDL - TRABAJO FINAL v1\n');
fprintf('============================================================\n');
fprintf('Geometria: L1=%.2f m, L2=%.2f m, L3=%.2f m\n', robot.L1, robot.L2, robot.L3);
fprintf('Modelo dinamico: M(q) qdd + C(q,dq)dq + G(q) = tau\n');
...
%% ================================================================
% 3. VALIDACION DEL MODELO DINAMICO
% ================================================================
...
%% ================================================================
% 4. PLANEACION AUTONOMA CON OBSTACULOS - A* EN PLANO XZ
% ================================================================
% Para que la presentacion sea visual y clara, se planifica en el plano XZ
% manteniendo Y constante. Luego cada punto cartesiano se convierte a q con IK.
% Esta es una decision defendible para un robot 3GDL que debe posicionar su
% efector evitando obstaculos en el espacio de trabajo.
...
% Punto inicial y final del efector. Deben ser alcanzables por IK.
start_xz = [0.35 0.25];      % [x z]
goal_xz  = [0.70 0.65];      % [x z]

% Obstaculos circulares: [x z radio]
obstacles = [0.48 0.40 0.07;
             0.60 0.52 0.06];

[path_xz, map] = astar_plan_xz(start_xz, goal_xz, obstacles, planner);

% Construccion de trayectoria cartesiana 3D desde la ruta A*.
path_cart = [path_xz(:,1)';
             planner.y_const*ones(1,size(path_xz,1));
             path_xz(:,2)'];

% Conversion ruta cartesiana -> ruta articular usando IK con continuidad.
...
% Visualizacion del mapa y la ruta planeada.
...
%% ================================================================
% 5. GENERACION DE TRAYECTORIA ARTICULAR SUAVE
% ================================================================
% Se interpola la ruta articular para obtener qd(t), dqd(t), ddqd(t).
% Los tres controladores seguiran la MISMA trayectoria para comparacion justa.

dt = 0.01;       % paso de simulacion [s]
tf = 8.0;        % duracion total [s]
traj = make_joint_trajectory(q_waypoints, tf, dt);

% Condicion inicial: cerca del primer punto, con un pequeno error inicial.
q0 = traj.qd(:,1) + deg2rad([8; -6; 5]);
qdot0 = [0; 0; 0];

%% ================================================================
% 6. DEFINICION DE GANANCIAS DE CONTROL
% ================================================================
% Ganancias preliminares. Deben ajustarse segun desempeno y estabilidad.

gains_pid.Kp = diag([80 90 70]);
gains_pid.Kd = diag([18 20 15]);
gains_pid.Ki = diag([8 8 6]);

% Para PD precompensado y par calculado se usan ganancias equivalentes.
...
%% ================================================================
% 7. SIMULACION CONTROL PID NO LINEAL
% ================================================================
% Ley usada:
%   tau = Kp*e + Kd*de + Ki*integral(e) + G(q)
% Se considera no lineal porque incorpora compensacion gravitacional G(q),
% que depende no linealmente de las coordenadas articulares.
...
%% ================================================================
% 8. SIMULACION PD CON PRECOMPENSACION
% ================================================================
% Ley usada:
%   tau = M(qd)*qddot_d + C(qd,dqd)*dqd + G(qd) + Kp*e + Kd*de
% Usa el modelo evaluado sobre la trayectoria deseada como precompensacion.

fprintf('2/3 PD con precompensacion...\n');
res_pd = simulate_robot_controller('PD_PRECOMP', robot, traj, q0, qdot0, gains_pd);

%% ================================================================
% 9. SIMULACION CONTROL POR PAR CALCULADO
% ================================================================
% Ley usada:
%   tau = M(q)*(qddot_d + Kd*de + Kp*e) + C(q,dq)*dq + G(q)
% Es el controlador mas dependiente del modelo, pero tambien el que suele
% lograr mejor seguimiento si M, C y G son razonablemente correctos.

fprintf('3/3 Control por par calculado...\n');
res_ct = simulate_robot_controller('PAR_CALCULADO', robot, traj, q0, qdot0, gains_ct);

%% ================================================================
% 10. METRICAS COMPARATIVAS
% ================================================================
...
%% ================================================================
% 11. GRAFICAS PARA PRESENTACION PRELIMINAR
% ================================================================
...
fprintf('\n================ RESUMEN EJECUTIVO ================\n');
fprintf('Se partio del trabajo parcial y se agrego el flujo completo solicitado.\n');
fprintf('Componentes incluidos: dinamica M,C,G; PID no lineal; PD precompensado; par calculado; A*.\n');
fprintf('Pendiente por confirmar con docente: parametros fisicos asumidos y obligatoriedad de Simulink.\n');
fprintf('Simulacion preliminar finalizada correctamente.\n');
